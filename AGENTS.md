...
- Catalog REST `/api/authors|genres|webtoons|episodes`. Coin packages `/api/coin-packages` (Impl 44). Comments `/api/comments` (Impl 45; soft-delete). Notifications `/api/notifications` (Impl 46; hard-delete). Settings `/api/settings` (Impl 47; four portal-safe fields). Writes on those four are super_admin or admin.
- Media REST `/api/media` (Impl 34 local disk; Impl 41 Cloudflare R2 when real env is set). A fake R2 key stays on disk.
...
- Admin SPA mock by default. `VITE_USE_MOCK_API=false` uses cookie + catalog REST (Impl 33), media REST (Impl 35), coin packages (Impl 44), comments (Impl 45), notifications (Impl 46), and settings (Impl 47). Auth split (Impl 49): `/login` and `/setup` share the split card; `/register` is not split.
