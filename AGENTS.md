...
- Data: `DataContext` + localStorage mock by default. When `VITE_USE_MOCK_API=false`, catalog + staff + media + coin packages + comments + reader users + notifications + reader broadcasts + the four portal-safe settings go through `src/lib/api/` (`credentials: 'include'`). Campaigns are not stuffed into `DataContext.notifications`. Reports stay mock. Theme and other Settings controls stay local.
