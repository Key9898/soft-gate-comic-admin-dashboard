...
export type PressNewsPatch = Partial<PressNewsWrite>;

export type PressStillRecord = {
  id: string;
  title: BilingualText;
  imageUrl: string;
  sortOrder: number;
...
export type PressStillPatch = Partial<PressStillWrite>;

export type PressStore = {
  getMeta: () => Promise<PressMetaRecord>;
  upsertMeta: (input: PressMetaRecord) => Promise<PressMetaRecord>;
  listNews: () => Promise<PressNewsRecord[]>;
  findNewsById: (id: string) => Promise<PressNewsRecord | null>;
  createNews: (input: PressNewsWrite) => Promise<PressNewsRecord>;
  updateNews: (id: string, patch: PressNewsPatch) => Promise<PressNewsRecord | null>;
  deleteNews: (id: string) => Promise<boolean>;
  listStills: () => Promise<PressStillRecord[]>;
  findStillById: (id: string) => Promise<PressStillRecord | null>;
  createStill: (input: PressStillWrite) => Promise<PressStillRecord>;
  updateStill: (id: string, patch: PressStillPatch) => Promise<PressStillRecord | null>;
  deleteStill: (id: string) => Promise<boolean>;
};

const bi = (en: string, mm: string): BilingualText => ({ en, mm });

export const DEFAULT_PRESS_META: PressMetaRecord = {
  copy: {
    intro: bi(
  ...
  facts: {
    legalName: {
      label: bi('Legal name', 'တရားဝင်အမည်'),
      value: bi('SoftGate', 'SoftGate'),
    },
    product: {
    ...
    market: {
      label: bi('Focus market', 'ဦးတည်ဈေးကွက်'),
      value: bi('Myanmar', 'မြန်မာ'),
    },
    hq: {
    ...
    founded: {
      label: bi('Founded', 'တည်ထောင်သည့်နှစ်'),
      value: bi('2026', '2026'),
    },
    platforms: {
      label: bi('Platforms', 'ပလက်ဖောင်းများ'),
      value: bi('Web', 'Web'),
    },
    languages: {
...
export function persistedCopy(row: PressCopy): PressCopy {
...
export function persistedFact(row: PressFact): PressFact {
  const next: PressFact = {
    label: persistedBilingual(row.label),
    value: persistedBilingual(row.value),
  };
  if (row.href) next.href = row.href;
  return next;
}

export function persistedFacts(
  row: Record<PressFactKey, PressFact>,
): Record<PressFactKey, PressFact> {
  const next = {} as Record<PressFactKey, PressFact>;
  for (const key of PRESS_FACT_KEYS) {
    next[key] = persistedFact(row[key]);
  }
  return next;
}

export function persistedPalette(row: PressPaletteSwatch[]): PressPaletteSwatch[] {
  return row.map((item) => ({ hex: item.hex, label: persistedBilingual(item.label) }));
}

export function persistedAssets(row: PressAsset[]): PressAsset[] {
  return row.map((item) => ({
    name: persistedBilingual(item.name),
    url: item.url,
    format: item.format,
  }));
}

export function persistedMeta(row: PressMetaRecord): PressMetaRecord {
  const next: PressMetaRecord = {
    copy: persistedCopy(row.copy),
    zipUrl: row.zipUrl,
    contactEmail: row.contactEmail,
    facts: persistedFacts(row.facts),
    palette: persistedPalette(row.palette),
    assets: persistedAssets(row.assets),
  };
  if (row.spokespersonMemberId) next.spokespersonMemberId = row.spokespersonMemberId;
  return next;
}

export function persistedNews(row: PressNewsRecord): PressNewsRecord {
  const next: PressNewsRecord = {
    id: row.id,
...
export function persistedStill(row: PressStillRecord): PressStillRecord {
  return {
    id: row.id,
    title: persistedBilingual(row.title),
