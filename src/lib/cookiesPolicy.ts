...
export const COOKIE_STORAGE_KEY = 'softgate_admin_cookies_v1';
export const COOKIE_SCHEMA_VERSION = 1;

export type CookieMeta = CookieMetaRecord;
export type CookieRow = CookieRowRecord;
export type CookieSnapshot = { meta: CookieMeta; rows: CookieRow[] };

export const COOKIE_COPY_LABELS: Record<CookieCopyKey, string> = {
  cookiesTitle: 'Page title',
  cookiesSeoDesc: 'SEO description',
  whatAreCookies: 'What are cookies',
  whatAreCookiesDesc: 'What are cookies body',
  howWeUseCookies: 'How we use cookies',
  howWeUseCookiesDesc: 'How we use cookies body',
  essentialCookies: 'Essential',
  essentialCookiesDesc: 'Essential body',
  functionalCookies: 'Functional',
  functionalCookiesDesc: 'Functional body',
  analyticsCookies: 'Analytics',
  analyticsCookiesDesc: 'Analytics body',
  marketingCookies: 'Marketing',
  marketingCookiesDesc: 'Marketing body',
  storageDetails: 'Storage details',
  storageDetailsDesc: 'Storage details body',
  managingCookies: 'Managing data',
  managingCookiesDesc: 'Managing data body',
  thirdPartyCookies: 'Third-party',
  thirdPartyCookiesDesc: 'Third-party body',
  updatesPolicy: 'Updates',
  updatesPolicyDesc: 'Updates body',
};

export const COOKIE_COPY_TEXTAREA = new Set<CookieCopyKey>([
  'cookiesSeoDesc',
  'whatAreCookiesDesc',
  'howWeUseCookiesDesc',
  'essentialCookiesDesc',
...
export function sortCookieRows(rows: CookieRow[]): CookieRow[] {
  return rows.slice().sort(compareCookieRow);
}

export function loadCookiesPolicy(): CookieSnapshot {
  const raw = localStorage.getItem(COOKIE_STORAGE_KEY);
  const seed = (): CookieSnapshot => ({
  ...
  try {
    ...
    if (
      parsed.schemaVersion !== COOKIE_SCHEMA_VERSION ||
      !parsed.meta ||
    ...
    return {
      meta: persistedCookieMeta(parsed.meta),
      rows: parsed.rows.map(persistedCookieRow),
    };
  } catch {
    return seed();
  }
...
export function saveCookiesPolicy(snapshot: CookieSnapshot): void {
  localStorage.setItem(
    COOKIE_STORAGE_KEY,
    JSON.stringify({
      schemaVersion: COOKIE_SCHEMA_VERSION,
      meta: persistedCookieMeta(snapshot.meta),
      rows: snapshot.rows.map(persistedCookieRow),
    }),
  );
}
