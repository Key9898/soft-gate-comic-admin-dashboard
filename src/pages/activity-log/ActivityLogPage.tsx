...
const mockActivityLogs: ActivityLog[] = [
  ...
  },
  {
    ...
    details: { en: 'Banned user for spam', mm: 'စပမ်းပို့ခြင်းကြောင့် အသုံးပြုသူအား ပိတ်ပင်ခဲ့သည်' },
...
const ActivityLogPage = () => {
  ...
  const getActionIcon = (action: string, type: string) => {
    ...
    const icons: Record<string, React.ReactNode> = {
      webtoon: <BookOpen className="w-4 h-4 text-purple-500" />,
      episode: <FileText className="w-4 h-4 text-blue-500" />,
  ...
  const getActionBadge = (action: string) => {
    ...
    return (
      <span
        className={`px-2 py-1 text-xs font-medium rounded-full ${styles[action] || 'bg-gray-100 text-gray-700'}`}
      >
            ...
            {filteredLogs.map((log) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  {log.details && <p className="text-sm text-gray-500 mt-1">{log.details.en}</p>}
                </div>
